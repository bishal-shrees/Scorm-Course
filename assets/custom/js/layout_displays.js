	...
	if (slide.questions[bookmark.current.question_id - 1].type == undefined || slide.questions[bookmark.current.question_id - 1] == "default") {
		...
		$('#module-content').html(
			Mustache.to_html($('#module-assessment-template').html(), {
				title: slide.questions[bookmark.current.question_id - 1].title,
				correct_answer: slide.questions[bookmark.current.question_id - 1].correct_answer,
				incorrect_answer: slide.questions[bookmark.current.question_id - 1].incorrect_answer,
				oid: function() {
					return oid++;
				},
				//answer : slide.questions[bookmark.current.question_id  - 1].answers[slide.questions[bookmark.current.question_id  - 1].answer].text,
...
function assessment_review(){
	$('#module-assessment-review-submit-btn').addClass('hidden');
	if(review == true){
		var count1 = 0;
		var count2 = 0;
	if (slide.questions[bookmark.current.question_id - 1].type == undefined || slide.questions[bookmark.current.question_id - 1] == "default") {
			var elements = document.getElementsByClassName("btn btn-answer");
				myMap.forEach(function(value, key){
					if(bookmark.current.question_id == key) {
							for(var j=0;j<elements.length;j++){
								if(j == value){
									$(elements[j]).addClass('active');
								}
								else {
										$(elements[j]).addClass('disabled');
								}
								if(current_slide.questions[bookmark.current.question_id - 1].answer == value){
									$('#review-answer-correct').removeClass('hidden');
								}
								else {
									$('#review-answer-incorrect').removeClass('hidden');
								}
						}
					}
				...
				$('#module-assessment-review-submit-btn').removeClass('hidden');
				$('#module-assessment-review-submit-btn').removeClass('disabled');
				$('#module-assessment-review-submit-btn').attr("disabled", false);
	}
	else if (slide.questions[bookmark.current.question_id - 1].type == "checkboxes") {
		myMap.forEach(function(value, key){
			if(bookmark.current.question_id == key) {
				$.each($("#checkboxes-answers input"), function(index, item) {
					...
					var answer = current_slide.questions[bookmark.current.question_id - 1].answers[id].check;
					if(answer ==true) {
						count1++;
					}
					for(var i=0;i<value.length;i++){
						if(id==value[i]){
							...
							if(current_slide.questions[bookmark.current.question_id - 1].answers[value[i]].check == true) {
								count2++;
							}
						}
						else {
				...
				$('#checkbox-review-answer-incorrect').removeClass('hidden');
				$('#checkbox-review-answer-correct').addClass('hidden');
			}
		});
		if(count1 == count2){
			$('#checkbox-review-answer-incorrect').removeClass('hidden');
			...
			$('#checkbox-review-answer-incorrect').addClass('hidden');
			$('#checkbox-review-answer-correct').removeClass('hidden');
		}
		if(notchecked == true){
			$('#checkbox-review-answer-incorrect').removeClass('hidden');
			$('#checkbox-review-answer-correct').addClass('hidden');
		...
		$("#drag_puzzle").empty();
			...
			if(bookmark.current.question_id == key) {
				...
				for(var j=0; j<value.length;j++){
					$("#drag_puzzle").append(value[j]);	
				}
				$.each($("#drag_puzzle li"), function(index, item){
					var id = $(item).attr("data-id");
					current_data = get_current_slide();
					console.log(current_data.questions[bookmark.current.question_id - 1].drags[id].answer-1+""+index);	
					if (index != current_data.questions[bookmark.current.question_id - 1].drags[id].answer-1 || $(item).attr("data-locked") != "1") {
							count1++;
							/**/		
						}
						else {
				...
				if(count1 > 0 ) {
					$('#drag-review-answer-correct').addClass('hidden');	
					$('#drag-review-answer-incorrect').removeClass('hidden');
				}
				else {
					$('#drag-review-answer-incorrect').addClass('hidden');
					$('#drag-review-answer-correct').removeClass('hidden');	
				}
			}
		});
		$('#module-drag-drop-puzzle-submit-assessment-btn').addClass('hidden');
		$('#module-drag-drop-puzzle-review-submit-assessment-btn').removeClass('hidden');
		 $('#drag_puzzle').sortable({ cancel: '#drag_puzzle li' });
		}
	}
}
...
function bootstrap_drag_drop_list_slide(slide, callback, in_assess) {
	...
	item_height = Math.floor($('#module-drag-drop-lists-selection').height() / 100 * slide.item_height) + "px";
		...
		if (slide.target.img != "") {
			...
			$('#drag-target').css("background-size", "contain");
...
function bootstrap_drag_drop_puzzle_slide(slide, callback, in_assess) {
	...
	$.each(slide.left, function(index, item) {
		...
		Mustache.to_html($('#module-drag-drop-puzzle-template').html(), {
			...
			drags: slide.drags,
			left : slide.left,
			correct_text: slide.correct_text,
			incorrect_text: slide.incorrect_text,
			section_id: section_id,
			slide_id: slide_id
		}));
...
function bootstrap_checkboxes_slide(slide, callback, in_assess) {
	...
	$('#module-content').html(
		Mustache.to_html($('#module-checkboxes-template').html(), {
			...
			incorrect_answer: slide.incorrect_answer,
			section_id: section_id,
			slide_id: slide_id
		}));
