...
$(function () {
  ...
  if (bookmark.is_started == false) {
    $("#module-content").html(
      Mustache.to_html($("#module-start-template").html(), {
  ...
  $(document).on("click", "#module-header-close", function () {
    $("#module-alert aside").html(
      Mustache.to_html($("#module-alert-response-content").html(), {
        title: "Are you sure?",
        textContent: "",
        responses: [
          {
  ...
  $(document).on("click", ".nav-module-slide", function () {
    ...
    if (current_slide.type == "video") {
      ...
      if (
        current_slide.slides[bookmark.current.scenario_pos - 1].type ==
        "scenario-video"
      ) {
        $current_video.dispose();
      }
  ...
  $(document).on(
    "change",
    "input[name=module-knowledge-check-answer]",
    function () {
  ...
  $(document).on("click", "#module-knowledge-check-submit-btn", function () {
    ...
    $current_question_id = $("#module-knowledge-check-content").data(
      "question-id",
    );
    $answer = $("input[name=module-knowledge-check-answer]:checked").val();
    current_slide = get_current_slide();
    if (
      current_slide.questions[$current_question_id - 1].answers[
        current_slide.questions[$current_question_id - 1].answer
      ].text == $answer
    ) {
  ...
  $(document).on(
    "click",
    "#module-knowledge-check-next-question-btn",
    function () {
      current_slide = get_current_slide();
  ...
  $(document).on(
    "change",
    "input[name=module-scenario-options-check-answer]",
    function () {
  ...
  $(document).on("click", "#module-scenario-option-continue-btn", function () {
    $("input[name=module-scenario-options-check-answer]").attr(
      "disabled",
      "disabled",
    );
  ...
  $(document).on("click", "#module-assessment-submit-btn", function () {
    ...
    myMap.set(bookmark.current.question_id, $answer);
    assessment_submit();
  });
  $(document).on("click", "#module-assessment-review-submit-btn", function () {
    assessment_submit();
  });
  ...
  $(document).on("click", "#module-assessment-review-btn", function () {
    review = true;
    bookmark.current.question_id = undefined;
    current_slide = get_current_slide();
    bootstrap_slide(current_slide, function () {});
  ...
  $(document).on(
    "click",
    "#module-drag-drop-targets-submit-assessment-btn",
    function () {
      current_data = get_current_slide();
      var current_slide =
        current_data.questions[bookmark.current.question_id - 1];
  ...
  $(document).on(
    "click",
    "#module-drag-drop-lists-submit-assessment-btn",
    function () {
      current_data = get_current_slide();
      var current_slide =
        current_data.questions[bookmark.current.question_id - 1];
  ...
  $(document).on(
    "click",
    "#module-drag-drop-puzzle-submit-assessment-btn",
    function () {
      current_data = get_current_slide();
      var current_slide =
        current_data.questions[bookmark.current.question_id - 1];
  ...
  $(document).on(
    "click",
    "#module-drag-drop-puzzle-review-submit-assessment-btn",
    function () {
      assessment_submit();
    },
  );
  ...
  $(document).on(
    "click",
    "#module-checkboxes-submit-assessment-btn",
    function () {
      current_data = get_current_slide();
      var current_slide =
        current_data.questions[bookmark.current.question_id - 1];
  ...
  $(document).on(
    "click",
    "#module-checkboxes-review-submit-assessment-btn",
    function () {
      assessment_submit();
    },
  );
...
function drag_drop_lists_tally(current_slide) {
  ...
  if (
    correct_ids >= current_slide.target.required_answers &&
    incorrect_ids == 0
  ) {
    return true;
  } else {
...
function drag_drop_puzzle_tally(current_slide) {
  ...
  $.each($("#drag_puzzle li"), function (index, item) {
    ...
    if (
      index != current_slide.drags[id].answer - 1 ||
      $(item).attr("data-locked") != "1"
    ) {
      correct = false;
    }
  });
...
function assessment_submit() {
  ...
  if (bookmark.current.question_id == current_slide.questions.length) {
    ...
    if (pass) {
      ...
      $("#module-assessment-review-btn").removeClass("hidden");
    } else {
      if (retryCount < 4) {
    ...
    for (slide_id in section.slides) {
      ...
      if (module_config.is_locked) {
        if (section_id < bookmark.furthest.section_id - 1) {
          slide_html +=
            "<a class='nav-module-slide' data-section-id='" +
            section_id_string +
            "' data-slide-id='" +
            slide_id_string +
            "' href='#'>" +
          ...
          section_id == bookmark.furthest.section_id - 1 &&
          slide_id <= bookmark.furthest.slide_id - 1
        ) {
          slide_html +=
            "<a class='nav-module-slide' data-section-id='" +
            section_id_string +
...
function is_last_slide() {
  if (
    bookmark.current.section_id >= get_section_count() &&
...
function get_next_slide() {
  ...
  if (
    bookmark.current.slide_id >=
    module_config.sections[bookmark.current.section_id - 1].slides.length
  ) {
    bookmark.current.section_id += 1;
    bookmark.current.slide_id = 1;
  } else {
...
function config_style() {
  ...
  $(".configBborderColor").css(
    "border-bottom-color",
    module_config.layout.borderColour[0],
  );
  $(".configTborderColor").css(
    "border-top-color",
    module_config.layout.borderColour[1],
  );
  $(".configBborderColor").css(
    "border-left-color",
    module_config.layout.borderColour[0],
  ...
  $(".configHLColor1").css(
    "background-color",
    module_config.layout.highlightColour[0],
  );
  $(".configHLColor2").css(
    "background-color",
    module_config.layout.highlightColour[1],
  );
  $("#config_styles").remove();
  $(
    "<style type='text/css' id='config_styles'>" +
      ".btn.btn-answer { color:" +
      module_config.layout.questionHighlightColour[0] +
      "; background-color:" +
      module_config.layout.questionBgColour[0] +
      "; border-color:" +
      module_config.layout.questionBorderColour[0] +
      ";} " +
      ".btn.btn-answer.active{ color:" +
      module_config.layout.questionHighlightColour[1] +
      "; background-color:" +
      module_config.layout.questionBgColour[1] +
      "; border-color:" +
      module_config.layout.questionBorderColour[1] +
      ";} " +
      ".dragable { color:" +
      module_config.layout.questionHighlightColour[0] +
      "; background-color:" +
      ...
      module_config.layout.questionBorderColour[0] +
      ";} " +
      ".drag_puzzle_item_bg_color { color:" +
      module_config.layout.questionHighlightColour[0] +
      "; background-color:" +
      module_config.layout.questionBgColour[0] +
      "; border-color:" +
      ...
      module_config.layout.questionHighlightColour[0] +
      "; } " +
      ".drag_puzzle_item { color:" +
      module_config.layout.questionHighlightColour[0] +
      "; }" +
      "input[type='checkbox']:checked + .indicator { background-color:" +
      module_config.layout.questionBgColour[1] +
      "; border-color:" +
      module_config.layout.questionBorderColour[1] +
      ";} " +
      "input[type='checkbox'] + .indicator { border-color:" +
      module_config.layout.questionBorderColour[1] +
      ";} " +
      ...
      ".popover.bottom .arrow:after {border-bottom-color: " +
      module_config.layout.questionBgColour[0] +
      ";}" +
      ".popover.top .arrow:after {border-top-color: " +
      module_config.layout.questionBgColour[0] +
      ";}" +
      ".popover.left .arrow:after {border-left-color: " +
      module_config.layout.questionBgColour[0] +
      ";}" +
      ".popover.right .arrow:after {border-right-color: " +
      module_config.layout.questionBgColour[0] +
      ";}" +
      "</style>",
  ).appendTo("head");
...
/**
 * Updates only progress for SCORM/xAPI registration
 * based on furthest slide visited.
 */
function update_progress() {
  if (!scorm || !module_config || !bookmark || !module_config.sections) return;

  var totalSlides = module_config.sections.reduce(
    (sum, sec) => sum + (sec.slides?.length || 0),
    0,
  );
  ...
  for (
    var i = 0;
    i <
    Math.min(bookmark.furthest.section_id - 1, module_config.sections.length);
    i++
  ) {
    completedSlides += module_config.sections[i].slides?.length || 0;
  }
  completedSlides += bookmark.furthest.slide_id || 0;
