...
function SCOBot(options) {
  ...
  var Utl = SCOBotUtil, // Hook for jQuery 'like' functionality
    defaults = {
      ...
      scorm_edition: "4th", // or 4th - this is a issue with "editions" of SCORM 2004 that differ
      scorm_status_persist: "completion_status", // could alternatively set it to 'completion_status' (*SCORM 1.2 only)
      ...
      completion_threshold: 1.0, // Override for default unless imsmanifest (LMS) has value
  ...
  function initSCO() {
    ...
    scorm.debug(
      settings.prefix + ": SCO Loaded from window.onload " + lmsconnected,
      4,
    );
    if (lmsconnected === "true") {
      ...
      var msg =
        "Sorry, unable to initialize the SCORM Runtime API. Returned: " +
        lmsconnected;
  ...
  function exitSCO() {
    scorm.debug(
      settings.prefix + ": SCO is being unloaded, forcing exit ...",
      3,
    );
  ...
  function isISO8601(v) {
    ...
    switch (scorm.get("time_type")) {
      case "UTC": // AT GMT
        iso8601Exp =
          /^(\d{4})-0?(\d+)-0?(\d+)[T ]0?(\d+):0?(\d+):0?(\d+)(?:\.(\d+))(|Z)$/;
        break;
      case "GMT": // FROM GMT
        iso8601Exp =
          /^(\d{4})-0?(\d+)-0?(\d+)[T ]0?(\d+):0?(\d+):0?(\d+)(?:\.(\d+))[\+\-]\d{2}:\d{2}$/;
        break;
  ...
  function notStartedYet() {
    scorm.debug(
      settings.prefix +
        ": You didn't call 'start()' yet, or you already terminated, ignoring.",
      2,
    );
    return "false";
  }
  ...
  function findResponseType(type, str) {
    ...
    switch (type) {
      ...
      default:
        scorm.debug(
          settings.prefix + ": Sorry, this is not a valid Response type.",
          1,
        );
        break;
    }
  ...
  function encodeInteractionType(type, value) {
    ...
    switch (type) {
      ...
      case "true-false":
        ...
        scorm.debug(
          settings.prefix +
            ": Developer, you're not passing true or false for true-false.  I got " +
            value +
            " instead",
          1,
        );
      ...
      case "choice":
         ...
         */
        if (scorm.getAPIVersion() === "1.2") {
          // not a fan of doing this but I didn't want to add more code.  JSLint will complain.
          if (Utl.isArray(value)) {
            if (value.length > 26 && settings.scorm_strict) {
              scorm.debug(
                settings.prefix +
                  ": Developer, you're passing a sum of values that exceeds SCORMs limit of 26 for this pattern.  Consider using 'performance' instead.",
                2,
              );
            }
            // Check char limit?
          ...
          scorm.debug(
            settings.prefix +
              ": Developer, you're not passing a array type for sequencing/choice.  I got " +
              typeof value +
              " instead\n" +
      ...
      case "sequencing":
        ...
        if (Utl.isArray(value)) {
          ...
          if (value.length > 36 && settings.scorm_strict) {
            scorm.debug(
              settings.prefix +
                ": Developer, you're passing a sum of values that exceeds SCORMs limit of 36 for this pattern.",
              2,
            );
          ...
          for (index in value) {
            if (value.hasOwnProperty(index)) {
              if (value[index].length > 10 && settings.scorm_strict) {
                scorm.debug(
                  settings.prefix +
                    ": Developer, you're passing values that exceed SCORMs limit of 10 characters.  Yours have " +
                    value[index].length +
                    ". I will truncate this as not to lose data.",
                  2,
                );
      ...
      case "fill-in":
        ...
        if (Utl.isPlainObject(value)) {
          ...
          if (Utl.isArray(value.words)) {
            // new error check
            str += value.words.join("[,]");
          } else {
            scorm.debug(
              settings.prefix +
                ": Developer, expected an array of word(s) for fill-in.  I got " +
                typeof value.words +
                " instead",
              1,
            );
          ...
          scorm.debug(
            settings.prefix +
              ": Developer, you're not passing a object type for fill in.  I got " +
              typeof value +
              " instead",
            1,
          );
      ...
      case "long-fill-in":
        ...
        if (Utl.isPlainObject(value)) {
          ...
          scorm.debug(
            settings.prefix +
              ": Developer, you're not passing a object type for long fill in.  I got " +
              typeof value +
              " instead",
            1,
          );
        ...
        if (Utl.isArray(value)) {
          ...
          while (i < len) {
            ...
            if (Utl.isArray(value[i])) {
              ...
              scorm.debug(
                settings.prefix +
                  ": Developer, you're not passing a array type for matching/performance.  I got " +
                  typeof value +
                  " instead",
                1,
              );
          ...
          str =
            scorm.getAPIVersion() === "1.2" ? arr.join(",") : arr.join("[,]");
          value = str;
        } else {
          scorm.debug(
            settings.prefix +
      ...
      case "performance":
        ...
        if (!Utl.isArray(value)) {
          // This would be a Correct Response Pattern
          // Check for order_matters
          if (value.order_matters !== undefined) {
            str += "{order_matters=" + value.order_matters + "}";
          }
            ...
            while (i < len) {
              if (Utl.isArray(value.answers[i])) {
                ...
                if (Utl.isPlainObject(value.answers[i][1])) {
                  arr2 = [
                    trueRound(value.answers[i][1].min, 7),
                    trueRound(value.answers[i][1].max, 7),
                  ];
                  str2 = arr2.join("[:]");
                ...
                scorm.debug(
                  settings.prefix +
                    ": Developer, you're not passing a array type for performance correct response.  I got " +
                    typeof value.answers[i] +
                    " instead on " +
                    i,
                  1,
                );
          ...
          if (Utl.isArray(value)) {
            // This would be a Learner Response Dev: had 'typeof' on it?
            len = value.length;
            i = 0;
            //for (i = 0; i < len; i += 1) {
            while (i < len) {
              if (Utl.isArray(value[i])) {
                ...
                scorm.debug(
                  settings.prefix +
                    ": Developer, you're not passing a array type for performance learner response.  I got " +
                    typeof value[i] +
                    " instead on " +
                    i,
                  1,
      ...
      case "numeric":
        if (typeof value === "number") {
          ...
          if (str === "NaN") {
            scorm.debug(
              settings.prefix +
                ": Developer, your not passing a number for a numeric interaction.  I got " +
                value +
                " instead",
              1,
            );
      ...
      default:
        ...
        scorm.debug(
          settings.prefix +
            ": Sorry, invalid interaction type detected for " +
            type +
            " on " +
            value,
          1,
      ...
      case "fill-in":
        ...
        if (arr !== null) {
          if (arr[0].search(/^\{case_matters=(true|false)\}$/) !== -1) {
            obj.case_matters = arr[0].substring(
              "{case_matters=".length,
              arr[0].length - 1,
            );
        ...
        if (arr !== null) {
          if (arr[0].search(/^\{order_matters=(true|false)\}$/) !== -1) {
            obj.order_matters = arr[0].substring(
              "{order_matters=".length,
              arr[0].length - 1,
            );
            value = value.substring(arr[0].length, value.length); // trim off
  ...
  function setSuspendData() {
    var result,
      ...
      data = settings.base64
        ? window.btoa(encodeURIComponent(cleansedData))
        : encodeURIComponent(cleansedData);
  ...
  function checkProgress() {
    if (scorm.isConnectionActive()) {
      ...
      if (settings.totalInteractions === 0 || settings.totalObjectives === 0) {
        ...
        scorm.debug(
          settings.prefix +
            ": Sorry, I cannot calculate Progress as the totalInteractions and or Objectives are zero",
          2,
        );
        return "false";
      }
      ...
      if (count > 0) {
        ...
        while (i >= 0) {
          ...
          tmpRaw = parseFloat(
            scorm.getvalue("cmi.objectives." + i + ".score.raw"),
          );
          ...
          if (!isNaN(tmpRaw)) {
            ...
            scorm.debug(
              settings.prefix +
                ": We got a NaN converting objectives." +
                i +
                ".score.raw.  This may be a global/local objective via the imsmanifest.xml.",
              2,
            );
          }
          if (
            scorm.getvalue("cmi.objectives." + i + ".completion_status") ===
            "completed"
          ) {
      ...
      if (parseFloat(buffer.score.max) - parseFloat(buffer.score.min) === 0) {
        ...
        scorm.debug(
          settings.prefix +
            ": Division by Zero for scoreMax - scoreMin " +
            buffer.score.max,
          2,
        );
        ...
        buffer.score.scaled =
          "" +
          trueRound(
            (buffer.score.raw - buffer.score.min) /
              (buffer.score.max - buffer.score.min),
            7,
          );
        scorm.debug(
          settings.prefix + ": Score Scaled = " + buffer.score.scaled,
          3,
        );
      }
      // Set Progress Measure ///////////////////
      buffer.progress_measure =
        "" + trueRound(totalObjectivesCompleted / settings.totalObjectives, 7);
      // Set Completion Status
      buffer.completion_status =
        parseFloat(buffer.progress_measure) >=
        parseFloat(buffer.completion_threshold)
          ? "completed"
          : "incomplete";
      ...
      scorm.debug(
        settings.prefix +
          ": Pass/Fail check - Calculated scaled score:" +
          parseFloat(buffer.score.scaled) +
          " vs. " +
          parseFloat(buffer.scaled_passing_score),
        3,
      );
      buffer.success_status =
        parseFloat(buffer.score.scaled) >=
        parseFloat(buffer.scaled_passing_score)
          ? "passed"
          : "failed";
  ...
  function updateStatus(ending) {
    if (ending && SCOBotManagedStatus) {
      scorm.debug(
        settings.prefix +
          ": I am finishing... storing score on finish? " +
          settings.doNotStatusUntilFinish,
        3,
      );
      if (settings.doNotStatusUntilFinish) {
        scorm.setvalue("cmi.score.raw", buffer.score.raw);
    ...
    if (!happyEndingRequest) {
      ...
      if (scorm.get("exit_type") === "finish" || ending) {
        if (storss !== defss && !isSuccessSet) {
          scorm.debug(
            settings.prefix + ": Overriding default success status to " + defss,
            3,
          );
        ...
        if (
          scorm.getAPIVersion() === "1.2" &&
          settings.scorm_status_persist === "completion_status" &&
          !isCompletionSet
        ) {
          ...
          if (storcs !== defcs && !isCompletionSet) {
            scorm.debug(
              settings.prefix +
                ": Overriding default completion status to " +
                defcs,
              3,
            );
            self.setvalue(cs, defcs);
  ...
  this.start = function () {
    ...
    if (!isStarted) {
      ...
      if (
         ...
         */
        settings.suspend_data = settings.base64
          ? decodeURIComponent(window.atob(scorm.getvalue("cmi.suspend_data")))
          : decodeURIComponent(scorm.getvalue("cmi.suspend_data")); // no longer unescaping
        // Quality control - You'd be surprised at the things a LMS responds with
        if (
          settings.suspend_data.length > 0 &&
          !isBadValue(settings.suspend_data)
        ) {
          // Assuming a JSON String
          scorm.debug(
            settings.prefix +
              ": Returning suspend data object from a prior session",
            4,
          );
          /* you may not be using JSON suspend data, and managing that yourself. */
          settings.suspend_data = settings.useJSONSuspendData
            ? JSON.parse(settings.suspend_data)
            : settings.suspend_data; // Turn this back into a object.
          scorm.debug(settings.suspend_data, 4);
          if (settings.entry === "") {
            settings.entry = "resume";
          ...
          scorm.debug(
            settings.prefix + ": Creating new suspend data object",
            4,
          );
        ...
        scorm.debug(
          settings.prefix +
            ": First time running this SCO based on LMS entry value.",
          4,
        );
      ...
      if (
        !isBadValue(tmpCompletionThreshold) &&
        tmpCompletionThreshold !== "-1"
      ) {
      ...
      if (
        !isBadValue(tmpScaledPassingScore) &&
        tmpScaledPassingScore !== "-1"
      ) {
        /*
        ...
        if (parseFloat(tmpScaledPassingScore > 1)) {
          buffer.scaled_passing_score = (
            (parseFloat(tmpScaledPassingScore) * 10) /
            1000
          ).toString();
        }
      ...
      if (settings.comments_from_lms !== "false") {
        ...
        Utl.triggerEvent(self, "comments_lms", {
      ...
      if (isISO8601Duration(settings.max_time_allowed)) {
        if (settings.initiate_timer) {
          scorm.debug(
            settings.prefix +
              ": This SCO has a set time, I am starting the timer for " +
        ...
        scorm.debug(
          settings.prefix +
            ": This is not ISO8601 time duration. " +
            settings.max_time_allowed,
        );
      }
    } else {
      scorm.debug(
        settings.prefix +
          ": You already called start!  I don't see much point in doing this more than once.",
        2,
      );
      return false;
    }
  ...
  this.setInteraction = function (data) {
    if (scorm.isConnectionActive()) {
      var version = scorm.getAPIVersion(),
        ...
        orig_timestamp =
          data.timestamp ||
          scorm.isoStringToDate(
            scorm.getvalue(
              p1 + scorm.getInteractionByID(data.id) + ".timestamp",
            ),
          ),
      ...
      if (!Utl.isPlainObject(data)) {
        scorm.debug(
          settings.prefix +
            ": Developer, your not passing a {object} argument!!  Got " +
            typeof data +
            " instead.",
          1,
        );
      ...
      if (isBadValue(data.id)) {
        ...
        scorm.debug(
          settings.prefix +
            ": Developer, your passing a interaction without a ID\nSee question:\n" +
            data.description,
          1,
        );
        for (key in data) {
      ...
      if (Utl.type(data.timestamp) === "date") {
        timestamp =
          scorm.getAPIVersion() === "1.2"
            ? scorm.dateToscorm12Time(data.timestamp)
            : scorm.isoDateToString(data.timestamp); // HH:MM:SS vs 2012-02-12T00:37:29
      }
      ...
      if (Utl.type(data.latency) === "date") {
        ...
        data.latency =
          scorm.getAPIVersion() === "1.2"
            ? scorm.centisecsToSCORM12Duration(latency * 100)
            : scorm.centisecsToISODuration(latency * 100, true);
      } else if (
        data.learner_response.length > 0 &&
        !isBadValue(data.learner_response)
      ) {
      ...
      if (data.objectives !== undefined) {
        ...
        while (i < len) {
          ...
          if (m === "false") {
            m =
              scorm.getvalue(p1 + p2) === "-1" ? "0" : scorm.getvalue(p1 + p2);
          }
          result = scorm.setvalue(
            p1 + "objectives." + m + ".id",
            data.objectives[i].id,
          );
          i += 1;
        }
      }
      ...
      if (Utl.isArray(data.correct_responses)) {
        ...
        while (j < len) {
          //for (j = 0; j < len; j += 1) {
          p = scorm.getInteractionCorrectResponsesByPattern(
            n,
            data.correct_responses[j].pattern,
          );
          scorm.debug(
            settings.prefix +
              ": Trying to locate pattern " +
              data.correct_responses[j].pattern +
              " resulted in " +
              p,
            4,
          ...
          if (p === "match") {
            scorm.debug(
              settings.prefix +
                ": Developer, I've already added this correct response type '" +
                data.correct_responses[j].pattern +
                "'",
              2,
            );
        ...
        scorm.debug(
          settings.prefix +
            ": Something went wrong with Correct Responses, it wasn't an Array.",
          1,
        );
      }
      ...
      if (!isBadValue(data.learner_response)) {
        if (version !== "1.2") {
          result = scorm.setvalue(
            p1 + "learner_response",
            encodeInteractionType(data.type, data.learner_response),
          );
        } else {
          result = scorm.setvalue(
            p1 + "student_response",
  ...
  this.getInteraction = function (id) {
    if (scorm.isConnectionActive()) {
      ...
      if (p !== "false") {
        ...
        while (i < p) {
          ...
          obj.correct_responses.push({
            pattern: decodeInteractionType(
              obj.type,
              scorm.getvalue(p1 + "correct_responses." + i + ".pattern"),
            ),
          });
          i += 1;
        }
      ...
      obj.learner_response = decodeInteractionType(
        obj.type,
        scorm.getvalue(p1 + "learner_response"),
      );
    ...
    if (scorm.isConnectionActive()) {
      ...
      if (isBadValue(n)) {
        ...
        if (n === "false") {
          scorm.debug(
            settings.prefix +
              ": LMS is return false, can not proceed, check error codes",
          );
          return n;
        }
        scorm.debug(
      ...
      if (f) {
        if (!isBadValue(data.id)) {
          ...
          scorm.debug(
            settings.prefix +
              ": You did not pass an objective ID!!  What I did get below:",
            1,
          );
          for (key in data) {
            if (data.hasOwnProperty(key)) {
      ...
      if (Utl.isPlainObject(data.score)) {
        if (version === "2004") {
          result = !isBadValue(data.score.scaled)
            ? sv(
                p1 + "score.scaled",
                trueRound(data.score.scaled, 7).toString(),
              )
            : scorm.debug(
                settings.prefix +
                  def1 +
        ...
        result = !isBadValue(data.score.raw)
          ? sv(p1 + "score.raw", trueRound(data.score.raw, 7).toString())
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        result = !isBadValue(data.score.min)
          ? sv(p1 + "score.min", trueRound(data.score.min, 7).toString())
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        result = !isBadValue(data.score.max)
          ? sv(p1 + "score.max", trueRound(data.score.max, 7).toString())
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        scorm.debug(
          settings.prefix +
            ": Did not receive a score object.  May or may not be an issue.",
          4,
        );
      }
      if (version === "2004") {
        result = !isBadValue(data.success_status)
          ? sv(p1 + "success_status", data.success_status)
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        result = !isBadValue(data.completion_status)
          ? sv(p1 + "completion_status", data.completion_status)
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        result = !isBadValue(data.progress_measure)
        ...
        result = !isBadValue(data.description)
          ? sv(p1 + "description", data.description)
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
        ...
        result = !isBadValue(data[settings.scorm_status_persist])
          ? sv(p1 + "status", data[settings.scorm_status_persist])
          : scorm.debug(
              settings.prefix +
                def1 +
                p1 +
                "status: " +
                data[settings.scorm_status_persist] +
                def2,
              3,
            );
      }
      scorm.debug(
        settings.prefix +
          ": Progress\n" +
          JSON.stringify(checkProgress(), null, " "),
        4,
      );
  ...
  this.setCommentFromLearner = function (msg, loc, date) {
    if (scorm.isConnectionActive()) {
      ...
      if (n === "false") {
        scorm.debug(
          settings.prefix +
            ": Sorry, LMS returned a comments count of 'false'.  Review error logs.",
        );
        return n;
      }
      if (msg.length === 0 || msg.length > 4000) {
        scorm.debug(
          settings.prefix +
            ": Sorry, message from learner was empty or exceeded the limit. Length:" +
            msg.length,
          2,
        );
      }
  ...
  this.gradeIt = function () {
    ...
    if (buffer.success_status !== "passed") {
      buffer.success_status =
        parseFloat(scoreScaled) >= parseFloat(buffer.scaled_passing_score)
          ? "passed"
          : "failed";
    }
    scorm.setvalue("cmi.success_status", buffer.success_status);
