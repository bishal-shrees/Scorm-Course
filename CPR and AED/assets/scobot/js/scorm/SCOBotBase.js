...
function SCOBotBase(options) {
  ...
  var Utl = SCOBotUtil,
    ...
    API = {
      ...
      data: {
        // Defaults, I'm moving a few of the SCORM defaults into this data object, they will be maintained here thereafter.
  ...
  function findAPI(win) {
    ...
    if (settings.preferred_API === "findSCORM12") {
      // Specifically 1.2
  ...
  function isoDateToStringUTC(d) {
    return (
      d.getUTCFullYear() +
      "-" +
      padTime(d.getUTCMonth() + 1) +
      "-" +
      padTime(d.getUTCDate()) +
      "T" +
      padTime(d.getUTCHours()) +
      ":" +
      padTime(d.getUTCMinutes()) +
      ":" +
      padTime(d.getUTCSeconds()) +
      "." +
      Math.round((d.getUTCMilliseconds() / 1000) % 1000) +
      "Z"
    );
  }
  ...
  function isoDateToString(d) {
    ...
    return (
      ...
      "." +
      Math.round((d.getMilliseconds() / 1000) % 1000) +
      offset +
      padTime(d.getTimezoneOffset() / 60) +
      ":00"
    );
    ...
    switch (settings.time_type) {
      ...
      case "GMT":
        d = str.replace(
          ...
          function ($0, $Year, $Month, $Day, $Hour, $Min, $Sec, $Ms, $Offset) {
      ...
      default:
        d = str.replace(
          ...
          function ($0, $Year, $Month, $Day, $Hour, $Min, $Sec) {
            return (
              MM[$Month - 1] +
              " " +
              $Day +
  ...
  function makeBoolean(str) {
    if (str === undefined) {
      debug(
        settings.prefix +
          " : makeBoolean was given empty string, converting to false",
        2,
      );
      return false;
    }
  ...
  this.getvalue = function (n) {
                    ...
                    if (!isNaN(parseInt(tiers[2], 10))) {
                      switch (tiers[3]) {
                        ...
                        default:
                          scorm.debug(
                            settings.prefix +
                              ": Requesting cmi.interactions." +
                              tiers.join(".") +
                              " is a write-only value.  Please check your code.",
                            2,
                          );
                  ...
                  default:
                    ...
                    switch (tiers[3]) {
                      ...
                      default:
                        scorm.debug(
                          settings.prefix +
                            ": Unexpected namespace passed in '" +
                            n +
    ...
    debug(
      settings.prefix +
        ": " +
        n +
        " Get Aborted, connection not initialized! " +
        API.isActive,
      2,
    );
    return "false";
  ...
  this.setvalue = function (n, v) {
    ...
    if (API.isActive) {
      ...
      switch (API.version) {
        case "1.2":
          API.mode =
            API.mode === ""
              ? lms.LMSGetValue("cmi.core.lesson_mode")
              : API.mode;
          ...
          switch (n) {
            ...
            case "cmi.comments_from_learner.comment":
              if (v.length > 4096) {
                debug(
                  settings.prefix +
                    ": Warning, the comment is over the limit!!",
                  2,
                );
              }
            ...
            case "cmi.location":
              if (v.length > 255) {
                debug(
                  settings.prefix +
                    ": Warning, your bookmark is over the limit!!",
                  2,
                );
              }
              nn = "cmi.core.lesson_location";
            ...
            case "cmi.exit":
              nn = "cmi.core.exit";
              if (v === "normal") {
                // Fix SCORM 1.2 doesn't have 'normal'
                v = "";
              }
              API.exit_type = v;
              break;
            ...
            case "cmi.suspend_data":
              if (v.length > 4096) {
                debug(
                  settings.prefix +
                    ": Warning, your suspend data is over the limit!!",
                  2,
                );
              }
              nn = n;
            ...
            default:
              ...
              switch (tiers[1]) {
                case "comments_from_learner":
                  switch (
                    tiers[3] // unsupported in SCORM 1.2
                  ) {
                    case "location":
                    case "timestamp":
                      return "false";
                ...
                case "interactions":
                  if (tiers[2] === "false") {
                    ...
                    scorm.debug(
                      settings.prefix +
                        ": Developer, the interaction count was 'false', and I'm stopping you from pushing this against the LMS.  Verify the code getting interaction count.",
                      2,
                    );
                    return "false";
                  }
                  switch (tiers[3]) {
                    ...
                    case "learner_response":
                      ...
                      break;
                    case "type":
                      if (v === "long-fill-in" || v === "other") {
                        // unsupported interaction types
                        //v = "fill-in"; // salvage it?
                        scorm.debug(
                          settings.prefix +
                            ": Developer, consider revising your recorded interactions.  SB.getAPIVersion() of '1.2' does not support type of " +
                            v,
                          2,
                        );
                      }
                ...
                case "objectives":
                  ...
                  switch (tiers[3]) {
                    ...
                    case "completion_status":
                      tiers[3] = "status"; // consolidate
                      if (v === "unknown") {
                        // not supported, fix - not attempted is allowed.
        ...
        case "2004":
          ...
          if (API.mode !== "normal") {
            debug(
              settings.prefix +
                ": Warning, you are not in normal mode.  The LMS may ignore 'SetValue' requests.",
              2,
            );
          }
          break;
    ...
    debug(
      settings.prefix +
        ": " +
        n +
        " Set Aborted, connection not initialized! Locate where you called it after you Terminated.",
      2,
    );
    return "false";
  };
      ...
      switch (API.version) {
        case "1.2":
          self.setvalue(
            ...
            centisecsToSCORM12Duration(session_secs * 100),
          );
        ...
        case "2004":
          self.setvalue(
            "cmi.session_time",
            centisecsToISODuration(session_secs * 100, true),
          );
          ...
          switch (API.data.completion_status) {
            ...
            default:
              if (API.data.completion_status === "") {
                triggerException(
                  "LMS compatibility issue, Please notify a administrator.  Completion Status is empty.",
                );
              }
              break;
          }
    ...
    if (API.isActive) {
      if (lms) {
        ...
        debug(
          settings.prefix +
            ": completion_status = " +
            API.data.completion_status +
            "|| success_status = " +
            API.data.success_status,
          3,
        );
      ...
      debug(
        settings.prefix + ": Terminate Aborted, connection not initialized!",
        2,
      );
    }
    return s;
  ...
  this.getObjectiveByID = function (id) {
    ...
    scorm.debug(
      settings.prefix +
        ": Set Objective - Begin search, Objective count is " +
        count,
      4,
    );
  ...
  this.getInteractionByID = function (id) {
    var count = self.getvalue("cmi.interactions._count"), // obtain total objectives
      ...
      tID;
    if (count === "" || count === "false" || count === "-1") {
      // could be unsupported in SCORM 1.2
      return "false";
    }
    if (API.version === "1.2") {
      ...
      scorm.debug(
        settings.prefix +
          ": Developer, consider ignoring these requests if SB.getAPIVersion is equal to 1.2",
        2,
      );
      return count;
    }
    count = parseInt(count, 10) - 1; // convert from string
    scorm.debug(
      settings.prefix + ": Getting interactions from count " + count,
      4,
    );
    i = count;
    //for (i = count; i >= 0; i -= 1) {
  ...
  this.getInteractionObjectiveByID = function (n, id) {
    ...
    count = parseInt(count, 10) - 1; // convert from string
    scorm.debug(
      settings.prefix + ": Getting interaction objectives from count " + count,
      4,
    );
    i = count;
    //for (i = count; i >= 0; i -= 1) {
    while (i >= 0) {
      ...
      if (id === tID) {
        scorm.debug(
          settings.prefix + ": Interaction Objective By ID Returning " + i,
        );
        return i;
      }
      i -= 1;
  ...
  this.getInteractionCorrectResponsesByPattern = function (n, pattern) {
    var count = self.getvalue(
        "cmi.interactions." + n + ".correct_responses._count",
      ), // obtain total correct_responses
      i,
      p;
    if (count === "" || count === "false") {
      scorm.debug(
        settings.prefix + ": Correct Responses pattern was empty or false",
        4,
      );
    ...
    scorm.debug(
      settings.prefix +
        ": Getting interaction correct responses from count " +
        count,
      4,
    );
    i = count;
    ...
    while (i >= 0) {
      p = self.getvalue(
        "cmi.interactions." + n + ".correct_responses." + i + ".pattern",
      );
      //scorm.debug(settings.prefix + ": Interaction Correct Responses Pattern Check for " + i + " : " + p + " vs " + pattern, 4);
      if (pattern === p) {
        scorm.debug(
          settings.prefix +
            ": Interaction Correct Responses By Pattern Returning " +
            i,
        );
        return "match";
      }
  ...
  this.init = function () {
    ...
    try {
      ...
      /* Cross Domain issue */
      debug(
        settings.prefix + " Possible Cross-domain issue/local mode (ignore).",
        2,
      );
      //debug(e, 1);
    }
    ...
    debug(
      settings.prefix + ": I was unable to locate an API for communication",
      2,
    );
    if (settings.use_standalone) {
      ...
      debug(
        settings.prefix +
          ": If you included Local_API_1484_11 I'll mimic the LMS.  If not, all SCORM calls will fail.",
        4,
      );
      ...
      API.path =
        typeof SCOBot_API_1484_11 === "function"
          ? new SCOBot_API_1484_11({ cmi: settings.cmi })
          : null;
